...
// Main exports
export function is_svg_valid(svg_data) {
    const ptr0 = passStringToWasm0(svg_data, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.is_svg_valid(ptr0, len0);
    return ret !== 0;
}

const QrScannerFinalization = (typeof FinalizationRegistry === 'undefined')
    ? { register: () => {}, unregister: () => {} }
    : new FinalizationRegistry(ptr => wasm.__wbg_qrscanner_free(ptr >>> 0, 1));

export class QrScanner {
    __destroy_into_raw() {
        const ptr = this.__wbg_ptr;
        this.__wbg_ptr = 0;
        QrScannerFinalization.unregister(this);
        return ptr;
    }

    free() {
        const ptr = this.__destroy_into_raw();
        wasm.__wbg_qrscanner_free(ptr, 0);
    }

    constructor(threshold) {
        const ret = wasm.qrscanner_new(threshold);
        this.__wbg_ptr = ret >>> 0;
        QrScannerFinalization.register(this, this.__wbg_ptr, this);
        return this;
    }

    scan_image(canvas, svg_image) {
        const ret = wasm.qrscanner_scan_image(this.__wbg_ptr, canvas, svg_image);
        let v1;
        if (ret[0] !== 0) {
...
export default async function init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('mid_qr_scanner_bg.wasm', import.meta.url);
    }
